...
export default function LineupSection({
        ...
        {logoSrc ? (
          ...
          <div className="text-center px-0.5 sm:px-2 flex items-center justify-center w-full overflow-hidden">
            ...
            {nameLower.includes('burgerkill') && (
              <span className="font-mono font-bold text-[8px] sm:text-sm md:text-lg tracking-normal sm:tracking-widest text-zinc-900 uppercase group-hover:text-pink-600 transition-colors whitespace-nowrap">
                &#123;BURGERKILL&#125;
              </span>
            )}
            {nameLower.includes('kotak') && (
              <span className="font-mono font-black text-[10px] sm:text-lg md:text-2xl tracking-tight sm:tracking-[0.2em] text-zinc-950 uppercase group-hover:text-pink-600 transition-colors">
                KOTAX
              </span>
            )}
            {nameLower.includes('geisha') && (
              <span className="font-serif font-semibold text-[7px] sm:text-xs md:text-base tracking-widest sm:tracking-[0.35em] text-zinc-900 uppercase group-hover:text-pink-600 transition-colors">
                GEISHA
              </span>
            )}
            {nameLower.includes('perunggu') && (
              <span className="font-sans font-black text-[7px] sm:text-xs md:text-lg tracking-tighter sm:tracking-wider text-zinc-950 uppercase group-hover:text-pink-600 transition-colors">
                PERUNGGU
              </span>
            )}
            {nameLower.includes('enau') && (
              <span className="font-sans font-black text-xs sm:text-xl md:text-3xl text-zinc-950 lowercase group-hover:text-pink-600 transition-colors">
                eňau
              </span>
            )}
            {nameLower.includes('charly') && (
              <div className="text-center leading-none">
                <span className="block font-serif italic text-[7px] sm:text-xs text-zinc-600">Charly</span>
                <span className="block font-sans font-black text-[6.5px] sm:text-[10px] md:text-xs tracking-tighter sm:tracking-wider uppercase text-zinc-950">SETIA BAND</span>
              </div>
            )}
            ...
            {nameLower.includes('t-five') && (
              <span className="font-sans italic font-black text-[10px] sm:text-lg md:text-2xl text-zinc-950 tracking-tight group-hover:text-pink-600 transition-colors">
                t-Five
              </span>
            )}
            {nameLower.includes('armand') && (
              <span className="font-sans font-black text-[6.5px] sm:text-[10px] md:text-xs tracking-tighter text-zinc-950 uppercase group-hover:text-pink-600 transition-colors">
                ARMAND MAULANA
              </span>
            )}
            {nameLower.includes('rony') && (
              <span className="font-serif font-bold text-[6.5px] sm:text-[10px] md:text-xs tracking-tighter sm:tracking-wider text-zinc-950 uppercase leading-tight group-hover:text-pink-600 transition-colors">
                RONY<br />PARULIAN
              </span>
            )}
            ...
            {nameLower.includes('changcuters') && (
              <span className="font-serif font-black text-[5.5px] sm:text-[9px] md:text-xs tracking-tighter sm:tracking-widest text-zinc-950 uppercase group-hover:text-pink-600 transition-colors">
                THE CHANGCUTERS
              </span>
            )}
            ...
            {nameLower.includes('andra') && (
              <span className="font-serif font-semibold text-[8px] sm:text-sm md:text-xl tracking-tighter sm:tracking-wider text-zinc-950 uppercase group-hover:text-pink-600 transition-colors">
                ANDRA <span className="text-[6px] sm:text-xs font-normal">AND THE</span> BACKBONE
              </span>
            )}
            ...
            {!['peterpan', 'burgerkill', 'kotak', 'geisha', 'perunggu', 'enau', 'charly', 'gigi', 't-five', 'armand', 'rony', 'letto', 'changcuters', 'kangen', 'andra'].some(k => nameLower.includes(k)) && (
              <span className="font-black text-[8px] sm:text-sm text-zinc-950 uppercase">
                {artist.name}
              </span>
            )}
          </div>
  ...
  return (
    <section className="pt-2 sm:pt-8 pb-16 sm:pb-20 px-2 sm:px-4 bg-white text-zinc-900">
      <div className="max-w-4xl mx-auto text-center flex flex-col items-center">
        
        {/* Phase Selector Badge / Pill Tabs */}
        {phases && phases.length > 1 && (
          <div className="inline-flex items-center justify-center gap-2 p-1.5 rounded-full bg-zinc-100/90 border border-zinc-200/80 shadow-xs mb-6 sm:mb-10 max-w-full overflow-x-auto">
            {phases.map((phase) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 sm:px-6 py-1.5 sm:py-2.5 rounded-full text-xs sm:text-xl font-bold transition-all duration-300 flex items-center gap-1 whitespace-nowrap ${
                    isSelected
                ...
                >
                  ...
                  <span className="text-zinc-950 font-black text-sm sm:text-xl ml-1">
                    {phase.name.replace(/phase/i, '').trim() || '1'}
                  </span>
                  <span className="text-zinc-800 font-semibold ml-1 text-xs sm:text-lg">
                    Lineup
                  </span>
                </button>
              );
        ...
        {/* 12-Column Compact Bento Grid Matching PLF26 Poster Layout on ALL devices (Mobile & Desktop) */}
        <div className="w-full max-w-4xl mx-auto grid grid-cols-12 gap-1.5 sm:gap-3 md:gap-4 auto-rows-[65px] xs:auto-rows-[75px] sm:auto-rows-[110px] md:auto-rows-[135px]">
          ...
          <div className="col-span-6 row-span-1 rounded-xl sm:rounded-3xl bg-transparent p-1.5 sm:p-5 flex flex-col justify-center text-left">
            <span className="text-zinc-700 font-medium text-xs sm:text-2xl tracking-tight leading-none mb-0.5 sm:mb-1">
              All Lineup
            </span>
            <div className="flex items-center gap-1 sm:gap-2">
              <span className="bg-gradient-to-r from-[#DB5494] via-[#8B5CF6] to-[#3B82F6] bg-clip-text text-transparent font-black text-xl sm:text-5xl leading-none">
                Phase
              </span>
              <span className="text-zinc-950 font-black text-xl sm:text-5xl leading-none">
                1 & 2
              </span>
            </div>
          </div>
          ...
          <div className="col-span-2 row-span-1 rounded-xl sm:rounded-3xl bg-gradient-to-b from-white to-zinc-50/90 border border-zinc-200/90 shadow-xs sm:shadow-md p-1 sm:p-2 flex items-center justify-center relative overflow-hidden group hover:scale-[1.02] transition-transform">
            <div className="w-8 h-8 sm:w-16 sm:h-16 rounded-full bg-gradient-to-tr from-pink-300 via-purple-300 to-indigo-300 p-0.5 sm:p-1 flex items-center justify-center shadow-lg shadow-pink-200/50 animate-pulse">
              <div className="w-full h-full rounded-full bg-gradient-to-tr from-purple-900 to-indigo-950 flex items-center justify-center border border-white/40">
                <div className="w-3 h-3 sm:w-6 sm:h-6 rounded-full bg-white/20 backdrop-blur-xs flex items-center justify-center border border-white/60">
                  <span className="text-white text-[7px] sm:text-[11px] font-black">««</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-6 row-span-1 rounded-xl sm:rounded-3xl bg-gradient-to-r from-zinc-100 via-white to-zinc-100 border border-zinc-200/90 shadow-xs sm:shadow-md p-2 sm:p-5 flex items-center justify-center overflow-hidden">
            <div className="flex items-center gap-0.5 sm:gap-1.5 h-6 sm:h-10 w-full justify-center opacity-70">
              {[40, 65, 30, 85, 50, 95, 40, 70, 30, 90, 60, 45, 80, 55, 90, 35, 75, 45, 85, 60, 30, 90, 50, 70, 40, 80].map((h, i) => (
                <div
                  key={i}
                  className="w-0.5 sm:w-1.5 bg-zinc-800 rounded-full transition-all duration-300"
                  style={{ height: `${h}%` }}
                />
              ))}
            </div>
          ...
          <div className="col-span-2 row-span-1 rounded-xl sm:rounded-3xl bg-transparent p-1 flex flex-col justify-center items-center text-center">
            <span className="bg-gradient-to-r from-[#DB5494] to-[#3B82F6] bg-clip-text text-transparent font-bold text-xs sm:text-2xl leading-none">
              More
            </span>
            <span className="text-zinc-800 font-medium text-[9px] sm:text-lg leading-tight">
              to Come
            </span>
          </div>
